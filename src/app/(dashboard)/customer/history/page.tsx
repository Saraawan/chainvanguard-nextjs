...
const mockOrderHistory = [
  {
    ...
    items: [
      { name: 'Premium Cotton T-Shirt', quantity: 2, price: 29.99, vendor: 'Fashion Hub' },
      { name: 'Wireless Bluetooth Headphones', quantity: 1, price: 199.99, vendor: 'Tech Solutions Inc.' },
      { name: 'Organic Coffee Beans', quantity: 1, price: 29.99, vendor: 'Green Farm Co.' }
    ],
  ...
  },
  {
    ...
    items: [
      { name: 'Yoga Exercise Mat', quantity: 1, price: 49.99, vendor: 'FitLife Store' },
      { name: 'Plant-Based Protein Powder', quantity: 2, price: 39.99, vendor: 'Nutrition Plus' },
      { name: 'Water Bottle', quantity: 1, price: 26.80, vendor: 'Eco Living' }
    ],
  ...
  },
  {
    ...
    items: [
      { name: 'Gaming Mouse', quantity: 1, price: 89.99, vendor: 'Tech Solutions Inc.' },
      { name: 'Mechanical Keyboard', quantity: 1, price: 159.99, vendor: 'Tech Solutions Inc.' },
      { name: 'Monitor Stand', quantity: 1, price: 74.97, vendor: 'Office Essentials' }
    ],
  ...
  },
  {
    ...
    items: [
      { name: 'Eco-Friendly Plates', quantity: 1, price: 45.99, vendor: 'Green Living Co.' },
      { name: 'Bamboo Cutlery Set', quantity: 1, price: 21.99, vendor: 'Eco Accessories' }
    ],
  ...
  },
  {
    ...
    items: [
      { name: 'Specialty Tea Collection', quantity: 1, price: 45.50, vendor: 'Tea Masters' }
    ],
    trackingId: null,
  ...
  },
  {
    ...
    items: [
      { name: 'Smart Watch', quantity: 1, price: 189.99, vendor: 'Tech Gadgets Pro' }
    ],
  ...
  },
  {
    ...
    items: [
      { name: 'Bluetooth Speaker', quantity: 1, price: 129.99, vendor: 'Audio Excellence' }
    ],
...
export default function OrderHistoryPage() {
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Ordered {formatDate(order.date)}</p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-gray-100">
            ...
            <div className="flex gap-1">
              {order.status === 'delivered' && !order.reviewed && (
                <Button variant="ghost" size="sm" className="h-7 text-xs px-2 text-blue-600 hover:text-blue-800">
  ...
  const totalSpent = mockOrderHistory.filter(o => o.status !== 'cancelled').reduce((sum, o) => sum + o.total, 0)
  const deliveredOrders = mockOrderHistory.filter(o => o.status === 'delivered').length
  const avgRating = mockOrderHistory.filter(o => o.rating).reduce((sum, o, _, arr) => sum + (o.rating || 0) / arr.length, 0)

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-medium text-gray-700">Order History</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Avg Rating', value: avgRating > 0 ? avgRating.toFixed(1) : 'N/A', icon: Star }
        ].map((stat, index) => {
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              {statusFilter !== 'All Status' && (
                <Badge variant="secondary" className="text-xs bg-gray-100 text-gray-600 gap-1">
                  {statusFilter}
                  <button onClick={() => setStatusFilter('All Status')} className="text-xs text-gray-400 hover:text-gray-600">×</button>
                </Badge>
              )}
            </div>
          </div>
      ...
      {filteredAndSortedOrders.length > 0 ? (
        ...
        <Card className="text-center py-8 border border-gray-100 bg-white">
          <CardContent>
            ...
            <h3 className="text-base font-medium text-gray-700 mb-2">No orders found</h3>
